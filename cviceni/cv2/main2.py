...
def main(argv):
    ...
    __test_image = cv2.imread('bmw_01_select/001111_20210319_111418354.jpg', 1)
    #__test_image = cv2.imread('bmw_01_select/000919_20210319_111410354.jpg', 1)
    __test_image_res = __test_image
    __test_image_only_one = cv2.imread('bmw_01_select/001111_20210319_111418354.jpg', 1)
    #cv2.imshow("img", __test_image)
    only_one_rect = []

    base_rec_size_arr = [150, 200, 250, 300, 350, 400]
    step_arr = [50, 100, 150]

    #base_rec_size_arr = [250]
    #step_arr = [50]

    for base_rec_size in base_rec_size_arr:
        for step in step_arr:

            print(f'base rec: {base_rec_size}, step: {step}')
            ...
            while 1:
                ...
                if result == 1:
                    #draw_rectangle(__test_image_res, top_left, bot_right)
                    positive_results += 1
                    rectList.append((top_left[0], top_left[1], bot_right[0], bot_right[1]))
            ...
            rectangles, weights = cv2.groupRectangles(rectList, 1)
            print(rectangles)
            print(weights)
            for rectangle in rectangles:
                top_left = (rectangle[0], rectangle[1])
                bot_right = (rectangle[2], rectangle[3])
                draw_rectangle(__test_image_res, top_left, bot_right)
                only_one_rect.append(rectangle)
            if len(rectangles) == 0 and positive_results == 1:
                top_left = (rectList[0][0], rectList[0][1])
                bot_right = (rectList[0][2], rectList[0][3])
                draw_rectangle(__test_image_res, top_left, bot_right)
                only_one_rect.append(rectList[0])


    only_one_rect_res, only_one_rect_wei = cv2.groupRectangles(only_one_rect, 1)
    print(only_one_rect_res)
    print(only_one_rect_wei)
    for rect in only_one_rect_res:
        # 677  569 1006  897]
        top_left = (rect[0], rect[1])
        bot_right = (rect[2], rect[3])
        draw_rectangle(__test_image_only_one, top_left, bot_right)


    result_resized = cv2.resize(__test_image_res, (1280, 720))
    cv2.imshow("result", result_resized)
    #cv2.waitKey()
    result_one_resized = cv2.resize(__test_image_only_one, (1280, 720))
    cv2.imshow("resulterino", result_one_resized)
    cv2.waitKey()
